...
func main() {
	...
	fmt.Printf("  交易对: %v\n", priceResult["symbol"])
	fmt.Printf("  价格: %v\n", priceResult["last_price"])
	fmt.Printf("  成交量: %v\n", priceResult["volume"])
	fmt.Printf("  更新时间: %v\n", priceResult["created_at"])
	...
	if price, ok := priceResult["last_price"].(float64); ok {
		fmt.Printf("\n价格分析:\n")
		fmt.Printf("  当前价格: %.8f\n", price)
		...
		if price >= gridLower && price <= gridUpper {
			...
			gridLevel := int((price - gridLower) / gridSpacing)
			if gridLevel >= 20 {
				gridLevel = 19
			}
			if gridLevel < 0 {
			...
			fmt.Printf("  超出百分比: %.2f%%\n", (price-gridUpper)/gridUpper*100)
		} else {
			fmt.Printf("  ❌ 价格低于网格下限 (%.4f < %.4f)\n", price, gridLower)
			fmt.Printf("  低于百分比: %.2f%%\n", (gridLower-price)/gridLower*100)
		}
	} else {
		fmt.Printf("  无法获取价格数据\n")
	}
	...
	techQuery := `
		SELECT symbol, rsi, macd, histogram, ma5, ma20, bb_width, volatility, trend
