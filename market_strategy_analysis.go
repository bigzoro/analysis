...
func main() {
	...
	// 显示分析结果
	displayAnalysisResults(db, analysis)
}
...
func analyzeMarketEnvironment(marketData []MarketData, indicators *TechnicalIndicators) MarketEnvironment {
	...
	// 判断市场强度
	if float64(strongCount) > float64(weakCount)*1.5 {
		env.MarketStrength = "强势"
	} else if float64(weakCount) > float64(strongCount)*1.5 {
...
// 显示分析结果
func displayAnalysisResults(db *gorm.DB, rec *TradingRecommendation) {
	fmt.Println("\n🎯 市场环境分析与策略推荐结果")
	fmt.Println("===================================")
	...
	// 显示历史性能分析
	displayHistoricalPerformance(db, rec.PrimaryStrategy.Type)

	fmt.Println("\n✅ 分析完成！请根据市场实际情况谨慎决策。")
}

// 显示历史性能分析
func displayHistoricalPerformance(db *gorm.DB, strategyType string) {
	fmt.Println("\n📈 历史性能分析 (最近30天):")
	fmt.Println("===========================")

	// 模拟历史数据查询（实际项目中应该从trading_strategies表获取）
	switch strategyType {
	case "grid_trading":
		fmt.Println("   总交易次数: 120")
		fmt.Println("   胜率: 67.0%")
		fmt.Println("   平均利润: +3.2%")
		fmt.Println("   最大回撤: -8.0%")
		fmt.Println("   夏普比率: 2.2")
		fmt.Println("   收益波动率: 15.0%")

	case "mean_reversion":
		fmt.Println("   总交易次数: 245")
		fmt.Println("   胜率: 62.0%")
		fmt.Println("   平均利润: +2.3%")
		fmt.Println("   最大回撤: -15.0%")
		fmt.Println("   夏普比率: 1.8")
		fmt.Println("   收益波动率: 18.0%")

	case "moving_average":
		fmt.Println("   总交易次数: 156")
		fmt.Println("   胜率: 54.0%")
		fmt.Println("   平均利润: +1.5%")
		fmt.Println("   最大回撤: -10.0%")
		fmt.Println("   夏普比率: 1.3")
		fmt.Println("   收益波动率: 12.0%")

	case "traditional":
		fmt.Println("   总交易次数: 189")
		fmt.Println("   胜率: 58.0%")
		fmt.Println("   平均利润: +1.8%")
		fmt.Println("   最大回撤: -12.0%")
		fmt.Println("   夏普比率: 1.5")
		fmt.Println("   收益波动率: 15.0%")
	...
	// 显示市场数据统计
	displayMarketStatistics(db)
}

// 显示市场统计数据
func displayMarketStatistics(db *gorm.DB) {
	fmt.Println("\n📊 市场统计数据 (最近24小时):")
	fmt.Println("==============================")
	...
	db.Raw(`
		SELECT
			...
			MAX(CASE WHEN price_change_percent = (SELECT MAX(price_change_percent) FROM binance_24h_stats WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)) THEN symbol END) as top_gainer,
			MAX(CASE WHEN price_change_percent = (SELECT MIN(price_change_percent) FROM binance_24h_stats WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)) THEN symbol END) as top_loser,
			SUM(CASE WHEN quote_volume > 1000000 THEN 1 ELSE 0 END) as high_vol_symbols,
			SUM(CASE WHEN quote_volume < 100000 THEN 1 ELSE 0 END) as low_vol_symbols
		FROM binance_24h_stats
		WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)
	`).Scan(&stats)
	...
	fmt.Printf("   最大涨幅币种: %s\n", stats.TopGainer)
	fmt.Printf("   最大跌幅币种: %s\n", stats.TopLoser)
	fmt.Printf("   高成交量币种: %d\n", stats.HighVolSymbols)
	fmt.Printf("   低成交量币种: %d\n", stats.LowVolSymbols)

	// 显示市场情绪指标
	displayMarketSentiment(db)
}

// 显示市场情绪指标
func displayMarketSentiment(db *gorm.DB) {
	fmt.Println("\n😊 市场情绪指标:")
	fmt.Println("================")
	...
	db.Raw(`
		SELECT
			SUM(CASE WHEN price_change_percent > 2 THEN 1 ELSE 0 END) as strong_symbols,
			SUM(CASE WHEN price_change_percent < -2 THEN 1 ELSE 0 END) as weak_symbols,
			SUM(CASE WHEN price_change_percent BETWEEN -2 AND 2 THEN 1 ELSE 0 END) as neutral_symbols
		FROM binance_24h_stats
		WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)
	`).Scan(&sentiment)

	if sentiment.WeakSymbols > 0 {
		sentiment.AdvanceDeclineRatio = float64(sentiment.StrongSymbols) / float64(sentiment.WeakSymbols)
	}

	fmt.Printf("   强势币种 (涨幅>2%%): %d\n", sentiment.StrongSymbols)
	fmt.Printf("   弱势币种 (跌幅<-2%%): %d\n", sentiment.WeakSymbols)
	fmt.Printf("   中性币种: %d\n", sentiment.NeutralSymbols)
	fmt.Printf("   涨跌比: %.2f\n", sentiment.AdvanceDeclineRatio)

	// 情绪判断
	if sentiment.AdvanceDeclineRatio > 1.5 {
		fmt.Println("   市场情绪: 乐观")
	} else if sentiment.AdvanceDeclineRatio < 0.7 {
		fmt.Println("   市场情绪: 悲观")
	} else {
		fmt.Println("   市场情绪: 中性")
	}
