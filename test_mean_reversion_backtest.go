...
import (
	...
	"analysis/analysis_backend/internal/analysis"
...
func backtestMeanReversionStrategy(cache *MarketDataCache, symbol string) (*BacktestResult, error) {
	...
	// 计算布林带
	upper, middle, lower := ti.CalculateBollingerBands(closes, period, bbMultiplier)
	...
	for i := period; i < len(klines); i++ {
		...
		// 检查是否需要平仓
		if position != 0 {
			holdHours := currentTime.Sub(entryTime).Hours()

			// 时间退出
			if holdHours >= float64(maxHoldHours) {
				exitReason := "持有时间超限"
		...
		if position == 0 {
			...
			if bbPosition > 0.8 && currentRSI > rsiOverbought {
				strength := calculateReversionStrength(bbPosition, currentRSI, 100-rsiOverbought, 0.8, 1.0)
				if strength >= minReversionStrength {
					position = -1
					entryPrice = currentPrice
					entryTime = currentTime
					stopLoss = entryPrice * 1.02  // 2%止损
					takeProfit = entryPrice * 0.96 // 4%止盈
...
func calculateStatistics(result *BacktestResult) {
	...
	if result.TotalTrades > 0 {
		...
		result.TotalPnLPercent = (totalPnL / 1000) * 100 // 假设初始资金1000
