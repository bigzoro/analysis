...
func NewStrategyBacktestEngine(db Database, dataManager interface{}) *StrategyBacktestEngine {
...
func (sbe *StrategyBacktestEngine) parseStrategyConfig(perf *pdb.RecommendationPerformance) (*StrategyConfig, error) {
	...
	if perf.StrategyConfig != nil {
		...
		// 对于空头策略，如果24h已经下跌超过15%，降低止盈目标以更快获利了结
		if config.StrategyType == "SHORT" && perf.Return24h != nil && *perf.Return24h < -15 {
			config.RiskParams.TakeProfitPercent = math.Min(config.RiskParams.TakeProfitPercent, 8.0)
			log.Printf("[STRATEGY] SHORT策略检测到强跌行情(24h跌幅%.2f%%)，调整止盈目标至%.1f%%",
				*perf.Return24h, config.RiskParams.TakeProfitPercent)
		}
	}
...
func (sbe *StrategyBacktestEngine) simulateStrategyExecution(
	...
	for i, kline := range klineData {
		...
		// 检查是否超过最大持仓时间 (默认24小时，避免过长的模拟时间)
		if positionOpen {
			holdingHours := klineTime.Sub(entryTime).Hours()
			if holdingHours >= float64(config.RiskParams.MaxHoldingHours) {
				log.Printf("[STRATEGY] 达到最大持有时间%.0f小时，强制退出: 入场价:%.4f, 当前价:%.4f, 收益:%.2f%%",
					holdingHours, entryPrice, kline.Close, (kline.Close-entryPrice)/entryPrice*100)
...
func (sbe *StrategyBacktestEngine) executeShortStrategy(
	...
	// 空头策略：当价格足够下跌时获利了结
	if currentReturn >= config.RiskParams.TakeProfitPercent {
		log.Printf("[STRATEGY] SHORT策略止盈触发: 当前收益%.2f%% >= 止盈目标%.2f%%, 入场价:%.4f, 当前价:%.4f",
			currentReturn, config.RiskParams.TakeProfitPercent, entryPrice, kline.Close)
		result.ExitPrice = kline.Close
		result.ExitTime = klineTime
		result.ExitReason = "profit"
		result.HoldingPeriodMinutes = int(klineTime.Sub(entryTime).Minutes())
	...
	// 2. 止损 (空头：价格上涨超过止损，亏损出场)
	if currentReturn <= -config.RiskParams.StopLossPercent {
		log.Printf("[STRATEGY] SHORT策略止损触发: 当前收益%.2f%% <= -止损%.2f%%, 入场价:%.4f, 当前价:%.4f",
			currentReturn, config.RiskParams.StopLossPercent, entryPrice, kline.Close)
		result.ExitPrice = kline.Close
		result.ExitTime = klineTime
		result.ExitReason = "loss"
		result.HoldingPeriodMinutes = int(klineTime.Sub(entryTime).Minutes())
...
// KlineData 复用数值类型的K线数据
type KlineData KlineDataNumeric
...
func (sbe *StrategyBacktestEngine) generateMockKlineData(symbol string, startTime, endTime time.Time, entryPrice float64, targetReturn24h float64) []KlineDataNumeric {
	data := make([]KlineDataNumeric, 0)
...
func (sbe *StrategyBacktestEngine) fetchRealKlineData(symbol, kind string, startTime, endTime time.Time) ([]KlineDataNumeric, error) {
	// 计算需要获取的数据范围
	...
	// 这里应该调用实际的K线数据获取API
	// 由于没有直接的API调用接口，这里返回nil，让调用方降级到模拟数据
