...
func (m *mockMarketDataProvider) GetMarketData(symbol string) (*execution.MarketData, error) {
	return &execution.MarketData{
		...
		HasSpot:     true,
		HasFutures:  true,
		// Set technical indicators for MA validation
		SMA5:  49000.0,
		SMA10: 48000.0,
		SMA20: 47000.0,
	}, nil
...
func (m *mockMarketDataProvider) GetKlineData(symbol, interval string, limit int) ([]*execution.KlineData, error) {
	// Return mock kline data for testing - need enough data for MA calculation
	klines := make([]*execution.KlineData, limit)
	for i := 0; i < limit; i++ {
		klines[i] = &execution.KlineData{
			OpenTime:   int64(1609459200000 + i*3600000), // 1 hour intervals
			CloseTime:  int64(1609462800000 + i*3600000),
			OpenPrice:  40000.0 + float64(i)*100,
			HighPrice:  42000.0 + float64(i)*100,
			LowPrice:   38000.0 + float64(i)*100,
			ClosePrice: 41000.0 + float64(i)*100,
...
func TestNewExecutor(t *testing.T) {
	...
	if executor.GetStrategyType() != "moving_average" {
		t.Errorf("Expected strategy type 'moving_average', got '%s'", executor.GetStrategyType())
	}
}
...
func TestExecutor_IsEnabled(t *testing.T) {
	...
	tests := []struct {
		name   string
		config *MovingAverageExecutionConfig
		want   bool
	}{
		{
			name: "enabled",
...
func TestExecutor_ValidateExecution(t *testing.T) {
	...
	// Get market data from provider (which includes SMA fields)
	marketData, err := deps.MarketDataProvider.GetMarketData("BTCUSDT")
	if err != nil {
