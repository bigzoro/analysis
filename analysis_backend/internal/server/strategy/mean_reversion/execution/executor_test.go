...
func (m *mockMarketDataProvider) GetMarketData(symbol string) (*execution.MarketData, error) {
	return &execution.MarketData{
		...
		Price:  50000.0,
		Volume: 1000.0,
		RSI:    65.0, // Set RSI for validation
	}, nil
}
...
func TestNewExecutor(t *testing.T) {
	...
	if executor.GetStrategyType() != "mean_reversion" {
		t.Errorf("Expected strategy type 'mean_reversion', got '%s'", executor.GetStrategyType())
	}
}
...
func TestExecutor_IsEnabled(t *testing.T) {
	...
	tests := []struct {
		name   string
		config *MeanReversionExecutionConfig
		want   bool
	}{
		{
			name: "enabled",
