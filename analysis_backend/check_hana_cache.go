...
func main() {
	...
	fmt.Printf("30秒前UTC时间: %s\n", nowUTC.Add(-30*time.Second).Format("2006-01-02 15:04:05"))

	for _, record := range hanaRecords {
		// 模拟调度器的新逻辑（使用UTC比较）
		ageUTC := nowUTC.Sub(record.LastUpdated.UTC())
		isFreshUTC := ageUTC <= 30*time.Second
		statusUTC := "❌ 过期"
		if isFreshUTC {
			statusUTC = "✅ 有效"
		}
		...
		// 旧逻辑（使用本地时间比较）
		ageLocal := now.Sub(record.LastUpdated)
		isFreshLocal := ageLocal <= 30*time.Second
		statusLocal := "❌ 过期"
		if isFreshLocal {
			statusLocal = "✅ 有效"
		}
		...
		fmt.Printf("    旧逻辑(本地时间): 缓存年龄 %v %s\n", ageLocal, statusLocal)
		fmt.Printf("    新逻辑(UTC时间): 缓存年龄 %v %s\n", ageUTC, statusUTC)
		fmt.Printf("    数据库时间: %s (UTC: %s)\n",
			record.LastUpdated.Format("2006-01-02 15:04:05"),
			record.LastUpdated.UTC().Format("2006-01-02 15:04:05"))
	}
