...
func main() {
	...
	// 1. 检查唯一的活跃Bracket订单
	fmt.Println("\n1️⃣ 检查活跃的XNYUSDT Bracket订单")
	var activeBracket pdb.BracketLink
	err = gdb.GormDB().Where("symbol = ? AND status = ?", "XNYUSDT", "active").First(&activeBracket).Error
	if err != nil {
		fmt.Printf("❌ 没有找到活跃的XNYUSDT Bracket订单: %v\n", err)
		return
	}
